...
# 📊 GameMetrics: AAA & Mobile Game LiveOps Player Telemetry Analytics

**AAA & Mobile Game LiveOps Telemetry Data Warehouse: D1/D7/D30 retention cohorts, battle pass conversion funnels, matchmaking tick rates, and ARPPU yield.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/GameMetrics-LiveOps-Player-Telemetry-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/GameMetrics-LiveOps-Player-Telemetry-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/GameMetrics-LiveOps-Player-Telemetry-Analytics)
...
**GameMetrics-LiveOps-Player-Telemetry-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Gaming LiveOps & Monetization Telemetry**.
...
### 1️⃣ Executive KPI Command Center (Cyberpunk Obsidian & Neon Amber (Dark))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_players      │──►│     fact_sessions       │◄──┤     dim_items_gacha  │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_game_modes   │   │     dim_platforms       │   │     dim_seasons      │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM gamemetrics_dw.fact_sessions f
JOIN gamemetrics_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/GameMetrics-LiveOps-Player-Telemetry-Analytics.git
cd GameMetrics-LiveOps-Player-Telemetry-Analytics
